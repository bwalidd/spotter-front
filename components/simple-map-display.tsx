...
export default function SimpleMapDisplay({ tripData, routeData }: SimpleMapDisplayProps) {
  const mapContainer = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (!mapContainer.current || !tripData || !routeData) {
      console.log('Simple map: Missing data or container')
      return
    }

    console.log('Simple map: Initializing with data:', { tripData, routeData })
    ...
    console.log('Simple map: Coordinates:', { currentCoords, pickupCoords, dropoffCoords })

    // Create a simple SVG map as fallback
    ...
    // Calculate positions for the three points
    const minLat = Math.min(currentCoords.lat, pickupCoords.lat, dropoffCoords.lat)
    const maxLat = Math.max(currentCoords.lat, pickupCoords.lat, dropoffCoords.lat)
    const minLng = Math.min(currentCoords.lng, pickupCoords.lng, dropoffCoords.lng)
    const maxLng = Math.max(currentCoords.lng, pickupCoords.lng, dropoffCoords.lng)

    const latRange = maxLat - minLat
    const lngRange = maxLng - minLng
    const padding = 0.1

    const scaleLat = (400 - 100) / (latRange + 2 * padding * latRange)
    const scaleLng = (1000 - 100) / (lngRange + 2 * padding * lngRange)

    const getX = (lng: number) => 50 + (lng - minLng + padding * lngRange) * scaleLng
    const getY = (lat: number) => 50 + (maxLat - lat + padding * latRange) * scaleLat

    const currentX = getX(currentCoords.lng)
    const currentY = getY(currentCoords.lat)
    const pickupX = getX(pickupCoords.lng)
    const pickupY = getY(pickupCoords.lat)
    const dropoffX = getX(dropoffCoords.lng)
    const dropoffY = getY(dropoffCoords.lat)

    // Draw route lines
    const line1 = document.createElementNS("http://www.w3.org/2000/svg", "line")
    line1.setAttribute("x1", currentX.toString())
    line1.setAttribute("y1", currentY.toString())
    line1.setAttribute("x2", pickupX.toString())
    line1.setAttribute("y2", pickupY.toString())
    ...
    line2.setAttribute("x1", pickupX.toString())
    line2.setAttribute("y1", pickupY.toString())
    line2.setAttribute("x2", dropoffX.toString())
    line2.setAttribute("y2", dropoffY.toString())
    line2.setAttribute("stroke", "#eab308")
    ...
    currentCircle.setAttribute("cx", currentX.toString())
    currentCircle.setAttribute("cy", currentY.toString())
    ...
    pickupLabel.setAttribute("y", (pickupY - 20).toString())
    ...
    dropoffLabel.setAttribute("x", dropoffX.toString())
    dropoffLabel.setAttribute("y", (dropoffY - 20).toString())
    ...
    // Add location names
    const currentName = document.createElementNS("http://www.w3.org/2000/svg", "text")
    currentName.setAttribute("x", currentX.toString())
    currentName.setAttribute("y", (currentY + 30).toString())
    currentName.setAttribute("text-anchor", "middle")
    currentName.setAttribute("font-size", "10")
    currentName.setAttribute("fill", "#374151")
    currentName.textContent = tripData.currentLocation
    svg.appendChild(currentName)

    const pickupName = document.createElementNS("http://www.w3.org/2000/svg", "text")
    pickupName.setAttribute("x", pickupX.toString())
    pickupName.setAttribute("y", (pickupY + 30).toString())
    pickupName.setAttribute("text-anchor", "middle")
    pickupName.setAttribute("font-size", "10")
    pickupName.setAttribute("fill", "#374151")
    pickupName.textContent = routeData.pickupLocation
    svg.appendChild(pickupName)

    const dropoffName = document.createElementNS("http://www.w3.org/2000/svg", "text")
    dropoffName.setAttribute("x", dropoffX.toString())
    dropoffName.setAttribute("y", (dropoffY + 30).toString())
    dropoffName.setAttribute("text-anchor", "middle")
    dropoffName.setAttribute("font-size", "10")
    dropoffName.setAttribute("fill", "#374151")
    dropoffName.textContent = routeData.dropoffLocation
    svg.appendChild(dropoffName)

    // Clear and add the map
    mapContainer.current.innerHTML = ""
    mapContainer.current.appendChild(svg)

    console.log('Simple map: Map created successfully')
  }, [tripData, routeData])

  return (
    <div className="space-y-4">
      <div ref={mapContainer} className="w-full h-96 rounded-lg border-2 border-orange-200 mb-10"></div>
