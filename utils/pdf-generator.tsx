...
export function generatePDF(tripData: any, routeData: any, logs: any[]) {
...
function generateCoverPage(tripData: any, routeData: any): string {
      ...
      <div class="trip-summary">
        ...
        <div class="summary-grid">
          <div class="summary-item">
            ...
            <value>${tripData.currentLocation}</value>
          </div>
          <div class="summary-item">
...
function generateLogPage(log: any, tripData: any, routeData: any, pageNum: number, totalPages: number): string {
    ...
    <div class="page">
      <div class="log-sheet">
        <div class="log-header">
          ...
          <div>
            <h3>Route</h3>
            <p>${tripData.currentLocation} → ${tripData.pickupLocation} → ${tripData.dropoffLocation}</p>
          </div>
        </div>
        ...
        <div class="log-content">
          <div class="hours-grid">
            ${Array(96)
              .fill(null)
              .map((_, segment) => {
                const hour = Math.floor(segment / 4)
                const minute = (segment % 4) * 15
                const activity = log.activities[segment]
                const color = activityColors[activity]
                const showLabel = segment % 4 === 0 // Only show hour labels for the first segment of each hour
                return `
                  <div>
                    <div class="hour-cell" style="background-color: ${color};">${minute === 0 ? hour : ''}</div>
                    ${showLabel ? `<div class="hour-label">${hour}</div>` : ''}
                  </div>
                `
              })
              .join("")}
          ...
          ${
            ...
            <div class="remarks">
              ...
              <div>
                ${log.remarks.map((remark: string) => `<div style="margin-bottom: 5px;">${remark}</div>`).join('')}
              </div>
            </div>
          `
              : ""
...
function generateSummaryPage(tripData: any, routeData: any, logs: any[]): string {
  ...
  return `
    <div class="page">
      ...
      ${getAllRemarks(logs).length > 0 ? `
      <div class="trip-summary">
        <h2>All Remarks</h2>
        ${logs.map((log, idx) => {
          if (!log.remarks || log.remarks.length === 0) return '';
          return `
            <div style="margin-bottom: 15px;">
              <div style="font-weight: bold; color: #1e40af; margin-bottom: 5px;">Day ${log.day} - ${log.date}:</div>
              <div class="remarks">
                ${log.remarks.map((remark: string) => `<div style="margin-bottom: 5px;">${remark}</div>`).join('')}
              </div>
            </div>
...
function calculateLogStats(activities: string[]): any {
  return {
    driving: activities.filter((a) => a === "driving").length / 4, // Convert 15-min segments to hours
    onDuty: activities.filter((a) => a === "on-duty").length / 4,
    sleeper: activities.filter((a) => a === "sleeper").length / 4,
    offDuty: activities.filter((a) => a === "off-duty").length / 4,
  }
}

function getAllRemarks(logs: any[]): string[] {
  const allRemarks: string[] = [];
  logs.forEach(log => {
    if (log.remarks && Array.isArray(log.remarks)) {
      allRemarks.push(...log.remarks);
    }
  });
  return allRemarks;
}
