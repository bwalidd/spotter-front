...
export function calculateAvailableHours(activities: string[], currentCycleUsed: number, dayIndex: number): number {
...
export function validateActivitySequence(activities: string[]): string[] {
  const errors: string[] = []
...
export function calculateDailyStats(activities: string[]): any {
  const stats = {
    driving: 0,
    onDuty: 0,
    offDuty: 0,
...
export function generateDefaultLog(day: number, date: string, estimatedDrivingHours: number): any {
...
export function calculateCycleUsage(logs: any[], initialCycleUsed: number): number {
  let totalDriving = initialCycleUsed
  logs.forEach((log) => {
    const drivingHours = log.activities.filter((a: string) => a === ACTIVITY_TYPES.DRIVING).length
